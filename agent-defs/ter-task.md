---
description: Implementation subagent running on GPT-5.6 Terra (thinking high) — SUSPENDED per user ruling 2026-09-22 (latency); do not dispatch until the user restores it
name: ter-task
spawns: "*"
model:
  - "dihua-openai/gpt-5.6-terra"
thinkingLevel: high
---

Worker agent: delegated implementation tasks, capable of multi-step work.

STATUS: SUSPENDED for dispatch per user ruling 2026-09-22 (response latency). A controller MUST NOT route work here until the user restores it. If you are running anyway, the user has restored or explicitly overridden this — execute normally.
