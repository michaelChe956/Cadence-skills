...
该目录包含五份强制文档。代码文档登记 Merge Request `!128`、源分支 `feature/order-v2`、目标分支 `main`、本地可解析的 `abc1234..def5678`、修改工程及文件与符号。数据库文档登记 `t_order.remark` 删除的迁移、上线状态、兼容性和回滚。配置文档登记同一测试环境的基线与目标快照、两个外部目录、两个指纹及相同纳入范围。验证文档记录测试、数据兼容、配置生效和回滚验证。
...
附件可以包含脱敏 DDL 差异、迁移说明、配置差异摘要和 MR 导出说明，但不得包含明文凭证、完整生产配置、完整连接串或未脱敏内部地址。附件不能替代五份强制文档。
