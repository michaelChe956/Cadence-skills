...
`change-summary.md` 的代码、数据模型、配置、中间件、接口、页面和业务知识七行必须齐全，状态只允许 `有变更` 或 `无变更`。每个 `无变更` 声明必须给出可审计的判断依据；“应该没变”“Git 没看到”“暂不清楚”不构成依据。
...
校验五份文档、七领域矩阵及相互一致性。代码、数据模型和配置状态必须分别与对应文档一致。中间件、接口和页面的 `无变更` 依据直接取自领域矩阵。业务知识的状态必须与 `business/` 下的 RULE/FLOW 文档及其 `条目状态` 一致：声明 `有变更` 时影响链必须触达对应 RULE/FLOW/术语候选条目，声明 `无变更` 时必须给出与既有条目一致的判断依据。
