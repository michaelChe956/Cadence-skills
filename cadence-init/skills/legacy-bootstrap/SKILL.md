...
| 模式 | 使用时机 | 产物范围 |
|------|----------|----------|
| 标准模式 | 默认模式，适合多数 legacy bootstrap | 生成总览、模块地图、构建测试画像、领域与数据初稿、风险区域、术语表、后续调研计划 |
| 深度模式 | 用户明确选择，或项目复杂且需要更细颗粒度认知 | 在标准模式基础上拆分更多模块、领域、风险和调研文档 |
...
- 用户选择深度模式：执行标准模式产物，并按证据补充模块、领域、风险和调研拆分文档。
- 用户未选择或未明确要求：执行标准模式。
