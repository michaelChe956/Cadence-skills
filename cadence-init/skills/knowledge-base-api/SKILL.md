...
- `表字段证据状态` 只描述字段定义来自 DDL、迁移、代码推导、用户提供、来源冲突或待确认；不能证明当前 API 已完整映射到该字段。
- `端到端映射状态` 单独判断 `API 模型 → SERVICE/MODULE → Mapper/SQL → TABLE 字段` 是否逐跳完整。缺少下游服务 Mapper/SQL、转换代码或任一关键跳时必须写 `待确认`，即使 DDL 已确认也不得升级。
...
字段、索引和约束以 `data-models/` 字段级文档及其表字段证据状态为准。API 文档只摘录与当前能力直接相关的字段影响并链接原表文档，不复制完整表结构；另行核实端到端映射状态，不能用表结构证据替代调用映射证据。
