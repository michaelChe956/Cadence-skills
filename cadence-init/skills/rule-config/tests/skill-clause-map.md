...
- fixture 命名（逐字）：`fx-<场景>`
- 条款编号：十张表沿用 design D2 行 ID 基线并新增 RF-05、SM-04、SM-05（NC-01~08、OS-01~08、L1-01~07、L0-01~07、RF-01~05、SM-01~05、OP-01~04、CS-01~08、CG-01~08、HM-01~03，共 64 行）；其余条款由本文档分配唯一编号（PM、FM、NR、NB、NH、DF、IA、CK、NX、S1a、S1b、S1d、L0-P、S3~S10、OS-N、CP 等前缀），编号在"条款摘要"列首标注。
...
| 增量运行-L0 入口（表 L0 + 屏障段） | 691-711 | L0-01~07、L0-B1~B2（当前 v2；旧版 v0、v1） | 9 |
| 增量运行-强制规则章节规范化（表 SM） | 713-717 | SM-01~05 | 5 |
...
| SKILL 行号区间 | 条款摘要 | 适用模式 | 脚本函数或 references 条目 | fixture | 测试 ID | 关键断言 |
|----------------|----------|----------|----------------------------|---------|---------|----------|
| 189 | L0-P1 读取模板根 `agent-routing-kernel.md` 当前 v2 完整内容作为规范源；v1 历史规范源固定为 `references/rules/l0-history/agent-routing-kernel-v1.md` | 两模式 | l0_block | fx-empty-project / fx-l0-old-version | TestL0V2Migration / ut-l0-v2-history-source / ut-l0_block-read-source | v2 区块 sha256 与当前规范源一致；v1 仅以冻结历史源作升级前比对 |
...
| 194 | L0-P6 当前 v2 标记成对且区块与规范源完全一致→跳过 | 两模式 | l0_block | fx-entry-idempotent | it-s4-idempotent / ut-l0-v2-skip | 两入口 sha256 不变；报告幂等跳过 |
| 195 | L0-P7 当前 v2 标记成对但区块不一致→视为本地修改：两模式统一纳入备份屏障，屏障通过后以规范源当前 v2 替换并报告（2026-08-19 权威化，原普通「无响应默认 keep」分支废止） | 两模式 | l0_block / backup_file | fx-l0-drift | it-s4-drift-normal-replaced / it-s4-drift-replaced-outside-preserved / ut-l0-v2-v1-drift | 当前标记漂移不得误判 current；两模式均归档后区块=规范源，区块外内容保留 |
...
| 197 | L0-P9 受支持旧版 v0、v1 成对标记→纳入备份屏障，屏障通过后升级为当前 v2；v1 必须与冻结规范源逐字一致，否则为 drift；v0 无规范源，成对即升级 | 两模式 | l0_block / backup_file | fx-l0-old-version | it-s4-upgrade / TestL0V2Migration / ut-l0-v2-upgrade / ut-l0-v2-v1-drift | v1 漂移不升级；v0 无规范源例外记入报告；可升级区块替换为 v2 |
| 198 | L0-P10 单侧/顺序错误→两模式统一按确定性安全归并处理（2026-08-19 权威化，原普通「询问/无响应默认 keep」子分支废止）；混合版本或重复区块→两模式确定性安全归并为单一 v2 区块 | 两模式 | l0_block | fx-l0-broken-markers | it-s4-broken-markers-preserve-arbitrary / TestL0V2Migration / ut-l0-v2-mixed / ut-l0-v2-dedup / ut-l0-v2-nested-broken | 完整安全配对才删区块；孤立标记不吞用户文本；重复 v2 报 `L0_DEDUP` |
| 199 | L0-P11 区块外项目技术栈、命令、业务规则和用户内容必须原样保留；污染对只剥离边界标记，不吞区块内用户正文 | 两模式 | l0_block | fx-l0-drift | it-s4-outside-preserved / ut-l0-v2-single / ut-l0-v2-overlap | 区块外 sha256 处理前后一致；污染对内部文本保留 |
| 200 | L0-P12 CLAUDE.md 与 AGENTS.md 必须使用相同 L0 版本和语义 | 两模式 | step_entry_files | fx-l0-drift | it-s4-dual-entry-consistency / TestL0V2Migration | 两入口受管区块 sha256 相同，均为 v2 |
...
| 635 | OS-N3 结构/类型不兼容→两模式统一先归档，成功后以模板内容原子替换；报告字段路径/实际类型（2026-08-19 权威化，原普通「保留不发布」分支废止） | 两模式 | precheck_openspec_structure / backup_file | fx-openspec-incompatible | it-s7-openspec-yaml-type-conflict-backed-up-replaced | 报告含字段路径与实际类型；归档存在；替换后原文件 sha256 变为目标模板 |
...
| 640 | OS-N8 禁止创建 `rules.apply`；已有 `rules.apply`：两模式统一先备份成功后在候选中移除并继续合并（2026-08-19 权威化，原普通「须确认/无响应保留」分支废止） | 两模式 | merge_yaml / backup_file | fx-openspec-apply-key | it-s7-openspec-apply-backed-up-removed / it-s7-openspec-normal-apply-removed | 两模式均备份后候选无 `rules.apply` 且合并继续；任何分支不虚构 apply artifact |
| 641 | OS-N9 YAML 无法可靠解析不得静默重写：两模式统一先归档，成功后以模板内容原子替换；仍无法构建候选则终止 | 两模式 | merge_yaml / backup_file | fx-openspec-unparseable | it-s7-openspec-invalid-yaml-backed-up-replaced | 归档存在；报告解析冲突；替换/终止分支均不虚构成功 |
...
| 659 | OS-N12 任一候选验证失败立即终止；报告失败字段路径、实际类型与错误（已废止 instructions 语义，由结构预检取代）；报告候选清理/保留结果；原文件不变；目标原本不存在时不得创建 | 两模式 | step_openspec_config / precheck_openspec_structure | fx-openspec-incompatible | `ut-s7-publish-or-abort-precheck-fail`（单测直接注入非法候选覆盖 `_s7_publish_or_abort` 候选 precheck fail-closed 分支）；集成侧 OS-03 结构冲突覆盖同代码路径 `it-s7-openspec-yaml-type-conflict-backed-up-replaced` | 单测：候选 precheck 失败→raise PublishError、原文件不变、动作日志含失败字段路径；集成：报告含结构预检失败字段路径与实际类型，无目标文件残留 |
...
| 668 | OS-03 目标字段结构/类型不兼容→两模式统一先归档，成功后以模板内容原子替换并报告；不再「保留不发布」 | 两模式 | precheck_openspec_structure | fx-openspec-incompatible | it-s7-openspec-yaml-type-conflict-backed-up-replaced | 两模式分支动作与报告字段符合表义；归档存在；替换后内容为目标模板 |
| 669 | OS-04 存在 `rules.apply`→两模式统一备份成功后在候选中移除并继续合并（原普通询问分支废止） | 两模式 | merge_yaml / backup_file | fx-openspec-apply-key | it-s7-openspec-apply-backed-up-removed / it-s7-openspec-normal-apply-removed | 两模式均备份后移除且合并继续；候选无 `rules.apply` |
| 670 | OS-05 YAML 无法可靠解析→两模式统一先归档，成功后以模板内容原子替换；归档失败则终止不改原文件（原普通「保留并报告」分支废止） | 两模式 | merge_yaml / backup_file | fx-openspec-unparseable | it-s7-openspec-invalid-yaml-backed-up-replaced | 归档存在；替换后内容为目标模板；报告解析冲突 |
...
| 684 | L1-04 仅旧版标记匹配但内容与旧版规范不同→归入"与任何已知框架版本不匹配"：两模式统一备份后以框架 v1 替换并报告（2026-08-19 权威化，原普通「无响应默认 keep」分支废止） | 两模式 | classify_l1 / backup_file | fx-l1-old-marker-drift | ut-classify_l1-old-marker-drift / it-s3-l1-drift-normal-replaced（旧版升级分支仅单测覆盖：仓库仅 v1 规范源，待补） | 判定 mismatch 而非 old-version；两模式均备份后替换为 v1 |
| 685 | L1-05 当前 v1 标记存在但完整内容不同→同归入"不匹配"，两模式统一备份后以框架 v1 替换 | 两模式 | classify_l1 / backup_file | fx-l1-v1-marker-drift | ut-classify_l1-v1-marker-drift / it-s3-l1-drift-normal-replaced / it-l1-drift-replace | 判定 mismatch；不得仅凭标记当作 current 跳过；两模式均备份后替换为 v1 |
| 686 | L1-06 文件无标记或与已知版本不匹配→两模式统一备份后以框架 v1 替换并报告（原普通「无响应默认 keep」分支废止） | 两模式 | classify_l1 / backup_file | fx-l1-unmarked | ut-classify_l1-unmarked / it-s3-l1-drift-normal-replaced / it-l1-unknown-replace | 判定 unmarked；两模式均备份后替换为 v1，不误当已知框架版本跳过 |
...
### 2.19 增量运行：L0 入口（L0-01~07，当前版本 v2）
...
| 699 | L0-01 入口不存在→创建基础入口并插入当前 v2（两模式同动作） | 两模式 | l0_block / step_entry_files | fx-empty-project | it-entry-base-created / TestL0V2Migration | 基础入口文本与 L0 v2 均存在，位置正确 |
| 700 | L0-02 当前 v2 区块与规范源完整一致→跳过不重复写入（两模式同动作） | 两模式 | l0_block / sha256_file | fx-entry-idempotent | it-s4-idempotent / ut-l0-v2-skip | 双入口 sha256 不变 |
| 701 | L0-03 当前 v2 标记成对但区块不同→视为无法识别的本地修改：两模式统一纳入备份屏障，屏障通过后以规范源当前 v2 替换并报告（原普通「无响应默认 keep」分支废止） | 两模式 | l0_block / backup_file | fx-l0-drift | it-s4-drift-normal-replaced / it-s4-drift-replaced-outside-preserved / ut-l0-v2-v1-drift | 当前标记漂移不得误判 current；两模式均备份后替换；区块外内容保留 |
| 702 | L0-04 受支持旧版为 v0、v1：v1 必须逐字匹配冻结 v1 规范源，v0 无规范源而以成对标记为升级例外；满足条件时备份成功后升级到当前 v2（两模式同动作） | 两模式 | l0_block / backup_file | fx-l0-old-version | it-s4-upgrade / ut-compute_plan-l0-upgrade-deterministic / TestL0V2Migration / ut-l0-v2-upgrade | 备份存在；不匹配 v1 判 drift；可升级区块=当前 v2 |
| 703 | L0-05 无 L0 标记→插入当前 v2，入口原内容保留（两模式同动作） | 两模式 | l0_block | fx-entry-no-markers | it-s4-insert / ut-compute_plan-l0-insert-deterministic / ut-step_s4-insert-normal-executes | 插入位置正确；原内容 sha256 不变 |
| 704 | L0-06 单侧/顺序错误→两模式统一按确定性安全归并处理；混合标记或重复区块→两模式安全归并为单一当前 v2 区块（原普通询问子分支废止） | 两模式 | l0_block | fx-l0-broken-markers | it-s4-broken-markers-preserve-arbitrary / TestL0V2Migration / ut-l0-v2-mixed / ut-l0-v2-dedup / ut-l0-v2-overlap | 仅安全成对区块可整块删除；孤立/污染标记不吞用户内容；重复当前块发出 `L0_DEDUP` |
...
| —（D3） | XC-03 decisions 异常校验（2026-08-19 起休眠兜底：当前系统无活跃冲突类型，六类受管冲突均转两模式确定性动作，不再生成决策冲突；机制保留供未来冲突类型复用）：文件缺失或无法解析 / 含未知或重复 `conflict_id` / 冲突缺少决策 / 决策与新鲜计划不符，任一即非零退出且零写入；计划无冲突时不要求决策文件 | 普通 | —（apply 入口决策校验） | fx-decisions-no-conflict | it-decisions-no-conflict-not-required / ut-validate-decisions-dormant（TestValidateDecisionsDormant 四例：unknown / duplicate / stale / missing-default-keep）/ ut-compute_plan-recommendation-keep（recommendation 一律保守 keep，终审 C-1） | 无冲突时不要求决策文件；休眠机制以合成冲突驱动 validate_decisions：未知/重复/过期决策各自失败关闭、缺省 default_keep 冲突不违规。原 it-decisions-unknown / it-decisions-stale 随受管 drift 决策冲突消亡移除（映射至休眠单测） |
...
| —（P1-1） | P1-1（已移除，2026-08-19）：受管规则 drift 冲突不再生成，s3/plan/报告的 `no_interrupt_action` 转发字段随之删除；普通与 no-interrupt 均不再出现该键；`recommendation=keep` 仅存于休眠兜底机制 | 两模式 | compute_plan / _sync_plan_to_report / run_dry_run | fx-existing-rules | 已移除（ut 登记：test_dry_run_no_interrupt_action_field / ut-compute-plan-no-interrupt-action / ut-compute-plan-normal-no-action-field / ut-report-no-interrupt-action / ut-report-normal-no-action-field 五个 ut 用例（TestDriftConflictNoInterruptAction 类内四例及类外 test_dry_run_no_interrupt_action_field）随 2026-08-19 权威化删除；受管 drift 动作由 it-s3-normal-authoritative-overwrite 与 RF-05 行覆盖） | 报告与 plan 不含 `no_interrupt_action` 键；受管 drift 动作统一为 `authoritative-overwrite` 且不经用户决策 |
...
| L0 入口增量 | 697-705（数据行 699-705） | 7 | L0-01~07 → 2.19 | ✅ 当前 v2、v0/v1 升级、混合标记安全归并均已对齐 |
| 规则文件处理 | 610-615（数据行 612-615，新增 RF-05 语义行） | 6 | RF-01~05 → 2.16 | ✅ RF-05 对齐框架权威全覆盖 |
| 强制规则章节规范化 | 713-717（数据行 715-717） | 5 | SM-01~05 → 2.20 | ✅ 创建、退役清理、重排、保留与 warnings 已一一对应 |
...
合计 64 行，全部映射到测试 ID；Task 9 文档对账以此为准。
...
| 2 | 框架受管规则 drift（两模式权威覆盖：屏障归档+模板替换） | it-s3-normal-authoritative-overwrite / ut-s3-authoritative-overwrite / ut-s3-authoritative-idempotent | ✅ 两模式权威覆盖与幂等均已覆盖（原普通 keep/replace 决策分支随权威化废止） |
...
| 16 | decisions 异常与无冲突豁免（休眠兜底） | it-decisions-no-conflict-not-required / ut-validate-decisions-dormant（XC-03，四例） | ✅ 无冲突豁免与休眠机制 fail-closed 均覆盖（原 unknown/stale 集成用例随决策冲突消亡移除） |
...
| 20 | no-interrupt dry-run 动作字段（2026-08-19 已移除：`no_interrupt_action` 键随受管 drift 冲突消亡删除） | 已移除（原 test_dry_run_no_interrupt_action_field 等 TestDriftConflictNoInterruptAction 全类 ut 删除；动作语义由 RF-05 行 `authoritative-overwrite` 承接） | ✅ 移除登记完成 |
