...
| CS-04 | 同上 | `.mcp.json` 有 CodeGraph MCP，但 `.codex/config.toml` 缺少 `[mcp_servers.codegraph]` | 参考 `.mcp.json` 手动补齐 `.codex/config.toml` | 同普通模式（两模式同动作） | 无（增量补 toml 块；按 §11.4 配置补齐不要求备份，除非破坏既有内容，破坏性写入仍按 §11.1） | 报告 toml 补齐的块；其余内容不变 | `it-s8-codegraph-toml-missing` |
