...
| L0-04 | 同上 | 受支持旧版本标记成对 | 备份成功后升级到当前 v2 并报告；v1 仅规范源逐字一致时可升级，v0 成对标记按无规范源例外升级 | 同普通模式（两模式同动作） | 将该入口纳入本次备份屏障（§11.2）；屏障失败双入口均不得写入 | 报告备份路径、旧版来源/例外及升级到 v2 | `it-s4-upgrade` / `ut-l0-v2-upgrade` / `ut-l0-v2-v1-drift` |
