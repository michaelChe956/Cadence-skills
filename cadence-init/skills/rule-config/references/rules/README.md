...
重新运行 `/cadence:init:rule-config` 会更新受管路由和已知版本框架规则。脚本以 dry-run / apply 两阶段执行：普通模式先 dry-run 探测冲突，就冲突逐条询问并把决策写入决策文件后再 apply；无响应或无决策时失败关闭，保留原文件并报告，确认替换的决策生效后由脚本在写入前统一备份。`no-interrupt` 模式单次 apply，冲突按权威规则处理，先备份、备份成功后再按框架规则替换。
