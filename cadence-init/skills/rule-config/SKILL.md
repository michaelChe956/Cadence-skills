...
4. **无响应处理**：无法等待用户输入或提问无响应时，Agent 必须把脚本给出的推荐默认决策**显式写入**决策文件——在 `/tmp` 生成 `<DECISIONS_JSON>`，内容为 JSON 数组，元素形如 `{"conflict_id": "<id>", "decision": "<推荐默认值>"}`，`decision` 取值必须落在该冲突的 `allowed_decisions` 内。不得在无决策文件的情况下直接 apply（脚本将失败关闭、零写入）。**具备安全默认的冲突**（仅限 `rules.apply` / OpenSpec 结构或类型不兼容 / YAML 无法解析 这类「无响应则保留原文件并报告」的 openspec 配置类条目，以及 report-only 的规则文件缺 CodeGraph 段落提示，详见 `references/merge-semantics.md` §11.6 A 类）即使决策文件缺该项，脚本亦按安全默认（keep / 保留原文件并报告）继续，不视为失败关闭；此类冲突在计划条目以 `default_keep: true` 标注。**无安全默认的冲突**（如**普通规则文件 drift**（RF-02b，既有规则文件内容与模板不一致）、L0/L1 关键路径 drift、类型矛盾）缺失决策仍 MUST 失败关闭——规则文件 drift 的「保留原状」并非脚本认可的安全默认，脚本不替用户做隐式 `keep` 决策，普通模式无响应或决策缺失即非零退出、零写入。
