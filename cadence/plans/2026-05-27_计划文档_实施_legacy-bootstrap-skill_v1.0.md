...
2. **确认执行模式** - 启动时询问是否使用深度模式；未选择则使用标准模式；轻量模式只在项目过大、时间有限或 repomix 输出不可处理时降级。
...
启动后必须询问用户是否使用深度模式；如果用户未选择深度模式，则使用标准模式。轻量模式不是常规三选一选项，只在项目过大、时间有限或 repomix 输出不可处理时作为降级方案。

| 模式 | 默认 | 说明 |
|------|------|------|
| 标准模式 | 是 | 按证据生成项目级 bootstrap 候选认知文档 |
| 深度模式 | 否 | 在标准模式候选产物基础上，按核心模块或领域继续拆分文档 |
| 轻量模式 | 否 | 仅在项目过大、时间有限或 repomix 输出不可处理时降级使用 |

默认推荐标准模式。如果用户选择深度模式，应按证据生成更多模块级、领域级和风险级文档。如果 repomix 输出过大或时间有限，可以建议降级到轻量模式。
````
...
1. 读取当前项目规则和入口文档。
2. 启动时确认是否使用深度模式；未选择则使用标准模式；轻量模式仅在 repomix 输出过大、不可处理或时间受限时降级。
...
实际文件以 skill 基于证据生成的结果为准。候选目录不代表必须全量生成；完成时至少汇报 repomix 输出文件、生成文档列表和入口文档更新情况。
