...
快照指纹固定为：对纳入范围内文件按相对路径排序，为每个文件计算 SHA-256，再对 `相对路径 + 制表符 + 文件 SHA-256` 的有序清单计算最终 SHA-256。Manifest 保存最终快照指纹、来源元数据和可审计范围摘要，包括 `scope_summary`、纳入文件数量或清单摘要、服务摘要与文件规则摘要；不保存原始快照、单文件内容哈希或敏感值哈希。重复文件哈希只在当前分析过程中临时使用。
...
- 日志配置归入可观测性。
- 相同内容文件合并分析并记录适用服务。
- 重复文件哈希只用于当前运行的去重，不写入 KnowledgeBase；Manifest 按 Task 1 契约保存最终快照指纹、来源元数据和可审计范围摘要，不保存原始快照、单文件内容哈希或敏感值哈希。
