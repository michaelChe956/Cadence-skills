...
```bash
rg -n "agent-routing-kernel.md|openspec-superpowers-workflow.md|L0 受管区块处理|OpenSpec 与 Superpowers 协作规则增量处理|cadence-backup-YYYYMMDDHHMMSS|契约与行为分层|失败关闭" cadence-init/skills/rule-config/SKILL.md
```

Expected: 所有新增入口、升级策略和核心原则均有匹配。
