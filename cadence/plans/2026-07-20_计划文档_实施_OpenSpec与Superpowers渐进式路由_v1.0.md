...
版本标记只用于候选版本定位；最终识别必须比较完整文件内容，不得仅凭标记把文件识别为已知旧版。
...
```bash
rg -n "agent-routing-kernel.md|openspec-superpowers-workflow.md|L0 受管区块处理|统一预检|全部必要备份成功后.*按各入口分支写入|CLAUDE.md 与 AGENTS.md 均不得写入|当前 v1 标记成对存在但完整受管区块.*无法识别的本地修改|版本标记受支持且完整文件内容与对应旧版规范逐字一致|仅受支持旧版本标记匹配但完整内容.*对应旧版规范不同|标记只用于候选版本定位.*最终识别必须比较完整文件内容|任何需要 L1 备份的分支备份失败.*不得替换原文件|OpenSpec 与 Superpowers 协作规则增量处理|当前 v1 标记存在但完整内容不同|与任何已知框架版本不匹配|cadence-backup-YYYYMMDDHHMMSS|契约与行为分层|失败关闭" cadence-init/skills/rule-config/SKILL.md
```

Expected: 所有新增入口、L0 原子屏障、L1 旧版完整内容识别、仅标记匹配的未知修改分支、备份失败保护、升级策略和核心原则均有匹配。
