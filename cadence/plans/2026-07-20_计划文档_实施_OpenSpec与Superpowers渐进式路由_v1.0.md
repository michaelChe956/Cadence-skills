...
3. 当前 v1 开始和结束标记成对存在，且完整受管区块与规范源当前 v1 完全一致时跳过。
4. 当前 v1 标记成对存在但完整受管区块与规范源当前 v1 不一致时，视为无法识别的本地修改；普通模式询问，无响应则保留并报告；`no-interrupt` 模式先创建可恢复的时间戳备份，成功后替换为规范源当前 v1 并报告。
5. 两个标记都不存在时，在首个 `## 强制规则` 前插入 L0；没有该标题时，在文件说明后插入。
6. 存在成对的受支持旧版本标记时，先创建时间戳备份，成功后升级为规范源当前 v1 并报告。
7. 只存在单侧标记或标记顺序错误时，普通模式询问后处理，无响应则保留并报告；`no-interrupt` 模式先创建时间戳备份，成功后写入单一 L0 区块并报告。
8. 任何 L0 备份失败时立即终止，且不得写入 L0 或改变入口文件的其他内容。
9. 区块外项目技术栈、命令、业务规则和用户内容必须原样保留。
10. CLAUDE.md 与 AGENTS.md 必须使用相同 L0 版本和语义。
```
...
```bash
rg -n "agent-routing-kernel.md|openspec-superpowers-workflow.md|L0 受管区块处理|当前 v1 标记成对存在但完整受管区块.*无法识别的本地修改|任何 L0 备份失败.*不得写入|OpenSpec 与 Superpowers 协作规则增量处理|当前 v1 标记存在但完整内容不同|与任何已知框架版本不匹配|cadence-backup-YYYYMMDDHHMMSS|契约与行为分层|失败关闭" cadence-init/skills/rule-config/SKILL.md
```

Expected: 所有新增入口、L0 同版本漂移保护、任意 L0 备份失败禁止写入、L1 同版本漂移归类、升级策略和核心原则均有匹配。
