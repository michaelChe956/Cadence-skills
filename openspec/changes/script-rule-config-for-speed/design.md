...
decisions.json schema（契约期冻结）：JSON 数组，元素为 `{"conflict_id": "<step>:<资产相对路径>[:<分支标识>]", "decision": "<枚举>"}`；`decision` 枚举按资产类型定义（规则文件/L0/L1：`replace|keep`；OpenSpec `rules.apply`：`remove_apply|keep`；其余分支以矩阵为准）。**决策文件机制仅适用普通模式**：普通模式 `apply` 开始时内部重算新鲜计划，计划无冲突时不要求决策文件，有计划内冲突时校验决策——文件缺失或无法解析、含未知或重复 `conflict_id`、决策与新鲜计划不符，任一发生即非零退出、写出报告且零写入。Agent 首选编排是写入推荐决策；对 `default_keep=true` 的 A 类冲突（凡 `recommendation=keep` 者：`rules.apply` / OpenSpec 结构或类型不兼容 / YAML 无法解析 / L0-03/06 / L1-04~06 / RF-02b 等），遗漏决策是合法的保留兜底（脚本按安全默认 keep 保留并报告 `status=0`）。详见 `references/merge-semantics.md` §11.6。codex 五轮重构：项目类型不再产生冲突（见下「项目类型判定两模式规则」），当前系统所有冲突均为 A 类（`default_keep` 保留兜底），无 B 类触发；`default_keep` 与 `validate_decisions` 机制代码保留兜底供未来冲突复用，但当前无 B 类 fail-closed 触发。no-interrupt 模式不读取也不要求决策文件，全部冲突按权威规则内部决策并记录报告。`--report` 与 `--decisions` 路径 MUST 位于项目根之外（建议 `/tmp`），脚本 SHALL 拒绝项目根内的报告/决策路径，保证 dry-run 零写入契约不被报告文件自身破坏。

流水线（每步独立计时、独立状态，失败即终止；**S8 为唯一例外**：install/init/status 失败按 degraded 降级继续）：

- **S1 detect**：有界 find 判项目类型（现行剪枝清单原样，只返回检测结果）+ 技术栈/包管理器检测（语言、test/lint/format 脚本，未检出写"未检测到"）；最终 `project_type` 按两模式唯⼀规则计算（no-interrupt 以检测为准、普通模式 CLI 仅提升；见下「项目类型判定两模式规则」），不再产生项目类型冲突。
...
**项目类型判定两模式规则**（codex 五轮重构）：脚本 `detect_project` 只返回检测结果（不读取 CLI），最终 `project_type` 由两模式唯⼀规则计算：no-interrupt 模式下 `final = 检测结果`（CLI `--project-type` 完全忽略）；普通模式下 `final = 'coding' if (检测=='coding' 或 CLI=='coding') else 'non-coding'`（CLI 只能把 non-coding 提升为 coding，检测为 coding 时无论 CLI 取何值均为 coding）。任一检测+CLI 组合都有唯⼀确定结果，不产生项目类型冲突，无需决策文件响应；S8 codegraph 启用条件 =（final `project_type=='coding'` 或 `--enable-codegraph`）。重要边界：不存在「non-coding 项目靠 `--project-type coding` 在 no-interrupt 装 codegraph」的场景（外层用户到不了该路径），不为它加任何逻辑或测试。
