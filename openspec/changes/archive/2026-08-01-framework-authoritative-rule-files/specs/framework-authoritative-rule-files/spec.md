...
术语定义：`cadence/legacy/` 是 rule-config 产生的被动恢复归档目录，仅用于存放被覆盖前的原文件副本，不属于任何 legacy 工作流插件遗留框架；协作路由与运行不依赖其中内容，与 `managed-rule-lifecycle` 中“规则层集成不得依赖 legacy”的 legacy（指任何 legacy 工作流插件）无关。
