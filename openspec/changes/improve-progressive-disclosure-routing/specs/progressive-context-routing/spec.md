...
对于需要读取仓库、创建或修改文件、调用 OpenSpec 命令、执行命令或声称完成的任务，系统 SHALL 要求 Agent 在首次工具调用前输出包含阶段、change、Plan 和必调 Skill 的简短路由回执。

#### Scenario: 开始实施已有 change
- **WHEN** Agent 准备实施名为 `sample-change` 的 OpenSpec change
- **THEN** 路由回执明确显示阶段为实施、change 名称、Plan 路径和执行类 Skill

#### Scenario: 纯概念问答
- **WHEN** 用户只要求解释概念且不需要读取仓库、创建文件、执行命令或声称完成
- **THEN** Agent 不必输出路由回执
- **AND** 不加载代码修改、文档存储或验证正文
