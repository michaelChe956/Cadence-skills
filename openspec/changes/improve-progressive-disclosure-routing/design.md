...
L1 文件属于 Cadence 框架规则，包含框架版本标识；识别到受支持旧版框架文件时备份后更新到新版。当前 v1 标记存在、但完整文件内容与框架规范源当前 v1 不同时，属于“与任何已知框架版本不匹配”，不得因版本标记相同而视为可直接覆盖：普通模式询问，无响应则保留原文件并报告，因为不覆盖所以无需额外备份；`no-interrupt` 模式备份成功后以框架当前 v1 替换并报告。任何需要 L1 备份的分支在备份失败时终止，且不得替换或改变原协作规则文件。
