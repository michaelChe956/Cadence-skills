...
`rule-config` 为 L0 使用稳定开始标记、结束标记和版本号，只处理受管区块，保留区块外项目内容。L0 完整受管区块与规范源当前 v1 完全一致时幂等跳过；当前 v1 开始和结束标记成对存在、但完整受管区块与规范源当前 v1 不一致时，视为无法识别的本地修改。普通模式询问是否替换，无响应则保留原区块并报告；`no-interrupt` 模式先创建可恢复的时间戳备份，备份成功后替换为规范源当前 v1 并报告。旧版本升级、单侧或乱序标记修复、同版本内容漂移处理等任意 L0 操作的备份失败时，必须终止且不得写入入口文件。

L1 文件属于 Cadence 框架规则，包含框架版本标识；识别到受支持旧版框架文件时备份后更新到新版。当前 v1 标记存在、但完整文件内容与框架规范源当前 v1 不同时，属于“与任何已知框架版本不匹配”，不得因版本标记相同而视为可直接覆盖：普通模式询问，无响应则保留并报告；`no-interrupt` 模式备份成功后以框架当前 v1 替换并报告；备份失败时终止。
