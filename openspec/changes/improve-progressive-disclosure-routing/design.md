...
`rule-config` 为 L0 使用稳定开始标记、结束标记和版本号，只处理受管区块，保留区块外项目内容。L0 完整受管区块与规范源当前 v1 完全一致时幂等跳过；当前 v1 开始和结束标记成对存在、但完整受管区块与规范源当前 v1 不一致时，视为无法识别的本地修改。普通模式询问是否替换，无响应则保留原区块并报告；`no-interrupt` 模式先创建可恢复的时间戳备份，备份成功后替换为规范源当前 v1 并报告。

同一次 `rule-config` 运行必须把 `CLAUDE.md` 与 `AGENTS.md` 作为一个 L0 更新单元：先统一预检两个入口的标记、版本、完整内容、目标动作和全部备份需求；在写入任一入口前，先成功创建本次所需的全部 L0 备份。旧版本升级、单侧或乱序标记修复、同版本内容漂移处理等任一必要备份失败时，立即终止本次 L0 更新，两个入口均不得写入；只有统一预检和全部备份成功后，才按各入口分支写入。写入后两个入口使用相同 L0 版本、保持语义等价，且各自区块外项目内容原样保留。

L1 文件属于 Cadence 框架规则，包含框架版本标识。版本标记只用于候选版本定位，最终识别必须比较完整文件内容：只有标记指向受支持旧版，且完整文件与该旧版规范逐字一致时，才识别为已知旧版并在备份后自动升级。当前 v1 标记存在但完整内容与框架规范源当前 v1 不同，或仅受支持旧版本标记匹配但完整内容与对应已知旧版不同，都属于“与任何已知框架版本不匹配”，不得仅凭标记自动升级或覆盖。普通模式询问，无响应则保留原文件并报告，因为不覆盖所以无需额外备份；`no-interrupt` 模式备份成功后以框架当前 v1 替换并报告。任何需要 L1 备份的分支在备份失败时终止，且不得替换或改变原协作规则文件。
