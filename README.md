...
- **默认关闭**：Agent 写完设计文档/实施计划或实现类产物（代码、测试、配置）后**禁止自动 `git commit`**，只汇报产物路径等待你确认——适合不想被 Agent 动 git 历史的项目；
- **开启自动提交**：把该行手改为 `：开启` 即可，之后 design/plan/code 写完会自动提交；
- **取值语义**：仅精确值 `开启` 视为启用，`关闭` 或任何其他值均按关闭处理；非法值保留原文不改写并在报告中告警；
- **旧名兼容**：旧名 `- **产物自动提交（design/plan）**：<值>` 会被识别为同一开关并自动迁移为新名、保留原值；
