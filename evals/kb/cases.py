...
SCENARIOS = {
    ...
    "V1": "调度与配置一致性（词汇不通：问『在不在跑』，代码里无此表述）",
    "V2": "规则存在但执行缺失（词汇不通：问『谁清理』，代码里只有保留期常量）",
    "V3": "跨域检索可行性（词汇不通：问『所有系统』，代码里只有 userId 关联）",
}
...
CASES = [
    ...
    # ═══ V 组：词汇不通（题面用词在代码中不出现，需跨文档综合或断言缺失）═══
    {
        "id": "V1-A", "scenario": "V1", "title": "每日对账到底在不在跑",
        "prompt": ("运营问：账户的每日对账任务到底有没有在跑？请给出结论与依据，"
                   "并说明若不生效缺什么。不要修改代码。"),
        "checks": [
            {"id": "V1A-1", "desc": "指出调度未被启用（缺 @EnableScheduling）",
             "require_all": ["EnableScheduling"]},
            {"id": "V1A-2", "desc": "指出 @Scheduled 存在但不会执行",
             "require_any": ["不会执行", "未生效", "不生效", "无法执行", "不会被调度", "不会触发"]},
            {"id": "V1A-3", "desc": "引用配置键 reconcile.enabled / retry-times",
             "require_any": ["reconcile.enabled", "retry-times", "reconcile"]},
            {"id": "V1A-4", "desc": "不武断断言任务正常运行",
             "forbid": ["对账任务正常运行", "任务已正常运行", "对账正常运行"]},
        ],
    },
    {
        "id": "V2-A", "scenario": "V2", "title": "导出文件到期谁清理",
        "prompt": ("运维问：订单导出文件保留 7 天这条规则在哪定义？到期后是谁清理的、"
                   "现在有没有真的在执行？"),
        "checks": [
            {"id": "V2A-1", "desc": "定位保留期来源",
             "require_any": ["RETENTION_DAYS", "7 天", "t_export_file"]},
            {"id": "V2A-2", "desc": "指出不存在清理实现（需断言缺失）",
             "require_any": ["没有清理", "未实现清理", "无清理", "不存在清理", "缺少清理",
                             "没有任何清理", "未发现清理", "没有实现"]},
            {"id": "V2A-3", "desc": "指出导出端点为存根",
             "require_any": ["存根", "taskId", "未调用 ExportService"]},
        ],
    },
    {
        "id": "V3-A", "scenario": "V3", "title": "按手机号跨域查用户记录",
        "prompt": ("客服拿到一个手机号，想一次看到这个用户在我们所有系统里的记录"
                   "（用户信息、账户、订单）。做得到吗？需要什么？有哪些还没有证据的地方？"),
        "checks": [
            {"id": "V3A-1", "desc": "指出手机号只在用户库",
             "require_any": ["t_user", "user-service", "只有用户", "仅在用户"]},
            {"id": "V3A-2", "desc": "关联必须经 userId",
             "require_any": ["userId", "user_id"]},
            {"id": "V3A-3", "desc": "无现成跨域聚合能力",
             "require_any": ["没有现成", "不存在现成", "需新建", "需要组合", "需新增", "没有聚合"]},
            {"id": "V3A-4", "desc": "账户唯一性/基数未证实",
             "require_any": ["唯一", "多条", "未证实", "无法保证", "未提供约束"]},
        ],
    },
]
